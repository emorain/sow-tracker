...
type AssignBoarHousingModalProps = {
  boar: Boar;
  onClose: () => void;
  onSuccess: () => void;
};

export default function AssignBoarHousingModal({ boar, onClose, onSuccess }: AssignBoarHousingModalProps) {
  const [housingUnits, setHousingUnits] = useState<HousingUnit[]>([]);
  const [selectedHousingId, setSelectedHousingId] = useState<string>(boar.housing_unit_id || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`${boar.ear_tag} ${selectedHousingId ? 'assigned to housing' : 'removed from housing'}`);
      onSuccess();
      onClose();
    } catch (err: any) {
            ...
            >
              ...
              {housingUnits.map((unit) => (
                <option key={unit.id} value={unit.id}>
                  ...
                  {unit.type && ` (${unit.type})`}
                  {unit.max_capacity && ` - ${unit.total_animals || 0}/${unit.max_capacity} animals`}
                </option>
              ))}
            </select>
