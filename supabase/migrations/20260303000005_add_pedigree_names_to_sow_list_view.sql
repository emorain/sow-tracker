-- Migration: Add sire_name and dam_name to sow_list_view
-- These fields were missing from the view, causing genetics data to disappear after save
...
SELECT
  ...
  -- Latest active farrowing date (for calculating nursing days)
  (
  ...
  -- Latest active breeding attempt - ID (for AI Dose button)
  (
    SELECT ba.id
    FROM breeding_attempts ba
    WHERE ba.sow_id = s.id
  ...
  -- Latest active breeding attempt - breeding cycle complete flag
  (
...
COMMENT ON VIEW sow_list_view IS 'Optimized view for sow list page - aggregates farrowing counts and breeding status to eliminate N+1 queries. Includes sire_name and dam_name for pedigree display.';
