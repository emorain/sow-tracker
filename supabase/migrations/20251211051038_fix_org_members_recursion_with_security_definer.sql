...
-- ANY query on organization_members that references organization_members causes recursion
...
-- Add comments
COMMENT ON FUNCTION is_organization_member(UUID, UUID) IS 
  'Helper function to check organization membership without RLS recursion. Uses SECURITY DEFINER to bypass RLS.';
COMMENT ON FUNCTION has_organization_role(UUID, UUID, TEXT[]) IS 
  'Helper function to check if user has specific role in organization. Uses SECURITY DEFINER to bypass RLS.';
