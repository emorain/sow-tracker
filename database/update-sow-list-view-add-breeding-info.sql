-- Update sow_list_view to include breeding_method and breeding_attempt_id
-- This allows us to show "Add AI Dose" button for AI-bred sows
...
SELECT
  ...
  -- NEW: Latest active breeding attempt - ID (for AI dose modal)
  (
    SELECT ba.id
    FROM breeding_attempts ba
    WHERE ba.sow_id = s.id
  ...
  -- NEW: Latest active breeding attempt - boar ID (for AI dose modal)
  (
    SELECT ba.boar_id
    FROM breeding_attempts ba
    WHERE ba.sow_id = s.id
...
COMMENT ON VIEW sow_list_view IS 'Optimized view for sow list page - aggregates farrowing counts and breeding status to eliminate N+1 queries. Includes breeding_method for AI dose tracking.';
